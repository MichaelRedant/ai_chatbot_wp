...
function octopus_ai_settings_page() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sitemap_url'])) {
        if (!current_user_can('manage_options') || !isset($_POST['octopus_ai_manual_sitemap_nonce']) || !wp_verify_nonce($_POST['octopus_ai_manual_sitemap_nonce'], 'octopus_ai_manual_sitemap')) {
            wp_die('Beveiligingsfout bij sitemap-URL.');
        }

        $new_url = esc_url_raw(trim($_POST['sitemap_url']));
        update_option('octopus_ai_sitemap_url', $new_url);
        ...
        <div class="source-mode-details">
            ...
            <div class="source-mode-panel <?php echo $source_strategy === 'sitemap_online' ? 'is-active' : ''; ?>" data-mode="sitemap_online">
                <div class="upload-box">
                    ...
                    <form method="post">
                        <?php wp_nonce_field('octopus_ai_manual_sitemap', 'octopus_ai_manual_sitemap_nonce'); ?>
